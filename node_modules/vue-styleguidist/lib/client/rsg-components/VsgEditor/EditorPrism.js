...
var highlight = function highlight(lang, jsxInExamples) {
  if (lang === 'vsg') {
    ...
    return function (code) {
      ...
      var htmlHighlighted = prismHighlight(SfcXMLDocument.body.innerHTML, langScheme, 'html');
      return htmlHighlighted.replace(/<span class="token language-javascript"> <\/span>/g, function () {
        var scriptBlock = scriptBlocks.shift();

        if (scriptBlock) {
          return "<span class=\"token language-typescript\">".concat(prismHighlight(scriptBlock.text, languages[scriptBlock.lg], scriptBlock.lg), "</span>");
        }
